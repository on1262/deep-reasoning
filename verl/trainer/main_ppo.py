...
from verl.utils.reward_score import kk_no_format
...
def _default_compute_score(data_source, solution_str, ground_truth):
    if "kk" in data_source:
        return kk_no_format.compute_score(solution_str, ground_truth)
    elif "simplelr" in data_source: # base model
        return hf_math_verify.compute_score(solution_str, ground_truth)
    elif "math_length" in data_source:
        return hf_math_verify.compute_score_with_length(solution_str, ground_truth)
...
class RewardManager():
    ...
    def __call__(self, data: DataProto):
        ...
        logger.info("RewardManager: reward_fn is called")
        reward_tensor = torch.zeros_like(data.batch['responses'], dtype=torch.float32)
        correctness_tensor = torch.zeros(len(data), dtype=torch.float32)
        already_print_data_sources = {}
        from tqdm import tqdm
        for i in tqdm(range(len(data)), desc="Rewarding", miniters=max(1, len(data) // 10)):
...
def main_task(config, compute_score=None):
    ...
    logger.info("[start init workers]")
