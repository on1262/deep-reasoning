...
class FSDPSFTTrainer(object):
    ...
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh):
        ...
        if self.device_mesh.get_rank() == 0:
            logger.debug(f'Using sequence parallel size: {self.config.ulysses_sequence_parallel_size}')
            logger.debug(f'Using remove padding: {self.use_remove_padding}')
    ...
    def _normalize_config_bsz(self):
        ...
        if self.device_mesh.get_rank() == 0:
            logger.debug(f'Normalize batch size by dp {dp_size}')
    ...
    def _build_dataloader(self):
        ...
        if self.config.ulysses_sequence_parallel_size > 1:
            ...
            if self.ulysses_device_mesh.get_rank() == 0:
                logger.debug(f'Using SP rank {rank} and size {world_size} for data distribution')
                logger.debug(f'Each SP rank gets different data, but the same data WITHIN the same rank')
        else:
            rank = self.device_mesh.get_rank()
            world_size = self.device_mesh.size()
        if self.device_mesh.get_rank() == 0:
            logger.debug(f'Using FSDP rank {rank} and size {world_size} for data distribution')
    ...
    def _build_model_optimizer(self):
        ...
        if self.config.optim.scheduler_type == 'cosine_with_warmup':
            self.lr_scheduler = get_cosine_schedule_with_warmup(optimizer=self.optimizer,
                                                            num_warmup_steps=num_warmup_steps,
                                                            num_training_steps=self.total_steps)
        elif self.config.optim.scheduler_type == 'constant_with_warmup':
        ...
        with context:
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=False):
                if not use_sp:
                    ...
                    raise NotImplementedError('micro batch size调整没有对sp处理')
                ...
                # NOTE: 在micro batch size > 1的时候, 之前处理不完全等价于micro batch size=1的情况
                # loss = torch.sum(loss) / valid_token_this_rank * dp_size
                # MODIFIED
                n_seq = input_ids.shape[0]
                valid_token_per_seq = torch.sum(loss_mask.view(n_seq, -1), dim=1)
                loss = torch.mean(torch.sum(loss.view(n_seq, -1), dim=1) / valid_token_per_seq) * dp_size
    ...
    def training_step(self, batch: TensorDict):
        ...
        if self.config.optim.clip_grad > 0:
    ...
    def save_checkpoint(self, step):
        ...
        with FSDP.state_dict_type(self.fsdp_model, StateDictType.FULL_STATE_DICT, cfg):
            state_dict = {k:v.to(torch.bfloat16) for k, v in self.fsdp_model.state_dict().items()}
    ...
    def fit(self):
        ...
        logger.debug(f'Total training steps: {self.total_training_steps}')
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            # save checkpoint
            if (epoch+1) % self.config.trainer.save_model_per_epoch == 0:
