...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        reward_tensor_lst = []
        data_source_lst = []
        assert len(self.val_dataloader) == 1 # KK 测试依赖last batch
        for test_data in tqdm(self.val_dataloader, 'validate'):
            test_batch = DataProto.from_single_dict(test_data)
            llogger.critical(f"test batch size 1: {len(test_batch)}")
            ...
            test_batch.batch['correctness'] = reward_tensor_dict['correctness_tensor']

            
            reward_tensor_lst.append(reward_tensor)
            correctness_lst.append(correctness)
            data_source_lst.append(test_batch.non_tensor_batch.get('data_source', ['unknown'] * reward_tensor.shape[0]))
        llogger.critical(f"test batch size 2: {len(test_batch)}")
        ...
        llogger.critical(f"test batch size 3: {len(test_batch)}")
        self.export_to_jsonl(test_batch, val_flag=True, export_eval_format=self.config.trainer.get('val_only', False))
        ...
        metric_dict.update(self.compute_level_metrics(test_batch, phase='val'))
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            logger.info(f"Create Actor Rollout")
        ...
        if self.use_reference_policy:
            logger.info(f"Create Ref Policy")
        ...
        logger.info(f"self.wg_dicts: {self.wg_dicts}")
        if self.use_critic:
        ...
        logger.info(f"Start creating vllm engine")
    ...
    def compute_level_metrics(self, batch: DataProto, phase='train'):
        from collections import defaultdict
        if not self.config.actor_rollout_ref.rollout.add_metrics_per_level: # 统计train/test set中不同level的正确率
            return {}
        ...
        for i in range(len(batch)):
            ...
            level = batch[i].non_tensor_batch['extra_info']['level']
            correctness = int(batch[i].batch['correctness'].item())
            ...
            is_clipped = int(not self.tokenizer.convert_ids_to_tokens(batch[i].batch['responses'][-1].item()) in eos_tokens)
            level_clip_ratio[level].append(is_clipped)
            # 计算每个level的总长度
            response_length = batch[i].batch['attention_mask'].sum().item() # 这一步实际算的是total length, 但是影响可以忽略不计
            level_response_lengths[level].append(response_length)

        # 计算每个level的正确率和clip_ratio
        for level, correctness_list in level_correctness.items():
            correctness = np.mean(correctness_list)
            metrics[f'{phase}_level_correctness/{level}_correctness'] = correctness
        for level, clip_ratio_list in level_clip_ratio.items():
            clip_ratio = np.mean(clip_ratio_list)
            metrics[f'{phase}_level_clip_ratio/{level}_clip_ratio'] = clip_ratio
        for level, response_length_list in level_response_lengths.items():
            response_length = np.mean(response_length_list)
            metrics[f'{phase}_level_response_length/{level}_response_length'] = response_length
        return metrics


    def export_to_jsonl(self, batch: DataProto, val_flag=False, export_eval_format=False):
        # non_tensor_batch['reward_model']['ground_truth']
        # export to jsonl
        os.makedirs(self.config.trainer.replay_buffer_dir, exist_ok=True)
        if not val_flag:
            rb_path = os.path.join(self.config.trainer.replay_buffer_dir, f'step_{self.global_steps}.jsonl')
        else:
            rb_path = os.path.join(self.config.trainer.replay_buffer_dir, f'val_step_{self.global_steps}.jsonl')

        llogger.info(f"Saving replay buffer to jsonl: {rb_path}")

        import jsonlines
        with jsonlines.open(rb_path, mode='w') as writer:
            #print('batch: ', batch)
            for i in tqdm(range(len(batch)), 'Export to jsonl. Decoding', miniters=max(1, len(batch) // 10)):
                ...
                # decode
                sequence = data_item.batch['input_ids'][prompt_length - valid_prompt_length:prompt_length + valid_response_length]

                seq_str = self.tokenizer.batch_decode([sequence])[0]

                ground_truth = data_item.non_tensor_batch['reward_model']['ground_truth']

                data_source = data_item.non_tensor_batch['data_source']

                correctness = data_item.batch['correctness'].item()

                if export_eval_format:
                    writer.write({
                        'prompt': 'None',
                        'responses': [seq_str],
                        ...
                        'extra_info': {
                            ...
                            'level': data_item.non_tensor_batch['extra_info']['level'],
        ...
        llogger.info("Save replay buffer to jsonl: Done")
    ...
    def fit(self):
        ...
        llogger.info("Check load checkpoint...")
        self._load_checkpoint()

        # perform validation before training
        # currently, we only support validation using the reward_function.
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', True):
            llogger.info("validating before train")
            ...
            llogger.info("Skip validation before train")

        # we start from step 1
        self.global_steps += 1

        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in tqdm(self.train_dataloader, f'Epoch {epoch}'):
                metrics = {}
                timing_raw = {}
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        """在这里计算Reward: 跳转到main_ppo RewardManager.__cal__()"""
                        llogger.info(f"Compute reward, reward_fn: {self.reward_fn}")
                        ...
                        self.export_to_jsonl(batch, val_flag=False)
                ...
                metrics.update(self.compute_level_metrics(batch, phase='train'))
