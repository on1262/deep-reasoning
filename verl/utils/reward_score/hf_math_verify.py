...
def extract_last_boxed(text):
    ...
    """
    # 经过测试, 这个正则可以捕获任意嵌套的 \boxed{} 命令, 包括内部有平衡的括号的情况
    pattern = r'\\boxed\{((?:[^{}]|\{(?:[^{}]|\{[^{}]*\})*\})*)\}'
...
def extract_solution(solution_str):
    ...
    # NOTE: 这里保留原版代码, extract_boxed_answer并没有用到
...
def compute_score_with_length(solution_str: str, ground_truth: str, method='strict'):
    eos_tokens = ['<|im_end|>', '<|endoftext|>', '<|finetune_right_pad_id|>', '<|eot_id|>', '<｜end▁of▁sentence｜>']
    if not any(solution_str.endswith(token) for token in eos_tokens):
        return {'score': 0.0, 'correctness': False}
    else:
        last_boxed_str = extract_last_boxed(solution_str)
        if last_boxed_str is None or last_boxed_str == '\\boxed{}': # 无匹配或者匹配到prompt
            return {'score': 0.0, 'correctness': False}
        elif last_boxed_str == '\\boxed{' + ground_truth + '}':
            return {'score': 1.0, 'correctness': True}
        else:
            return compute_score(last_boxed_str, ground_truth, method=method)

def compute_score_gsm8k(solution_str: str, ground_truth: str, check_eos=False):
    eos_tokens = ['<|im_end|>', '<|endoftext|>', '<|finetune_right_pad_id|>', '<|eot_id|>', '<｜end▁of▁sentence｜>']
    if check_eos and (not any(token in solution_str for token in eos_tokens)):
        return {'score': 0.0, 'correctness': False}
    else:
        text = extract_last_boxed(solution_str)
        if not isinstance(text, str):
            return {'score': 0.0, 'correctness': False}
        # 只取text中的数字和小数点
        text = re.sub(r'[^0-9.]+', '', text)
        try:
            correct = (int(float(text)) == int(ground_truth))
        except ValueError:
            # 如果转换失败，说明不是数字
            return {'score': 0.0, 'correctness': False}
        # 如果转换成功，比较数字
        return {'score': 1.0 if correct else 0.0, 'correctness': correct}
