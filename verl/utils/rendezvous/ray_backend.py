...
def get_nccl_id_store_by_name(name):
    ...
    if len(matched_actors) == 1:
        ...
        logger.warning(f"multiple actors with same name found: {matched_actors}")
    elif len(matched_actors) == 0:
        logger.info(f"failed to get any actor named {name}")
    return None
    ...
    if rank == 0:
        ...
        for i in range(max_retries):
            ...
            if nccl_id_store is not None:
                logger.info(f"nccl_id_store {group_name} got")
                nccl_id = ray.get(nccl_id_store.get.remote())
                logger.info(f"nccl id for {group_name} got: {nccl_id}")
                communicator = NcclCommunicator(
                    ndev=world_size,
                    commId=nccl_id,
                    rank=rank,
                ...
                return communicator
            logger.info(f"failed to get nccl_id for {i+1} time, sleep for {interval_s} seconds")
