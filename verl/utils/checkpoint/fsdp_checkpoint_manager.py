...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, global_step: int, remove_previous_ckpt=True, *args, **kwargs):
        ...
        if not remove_previous_ckpt: # NOTE: 这个参数被改写了, 如果为False, 会保存checkpoint并移除previous checkpoint, 如果为true, 只保存model
        ...
        if self.rank == 0:
            if remove_previous_ckpt:
                ...
                hf_local_path = local_path # 没有hf子文件夹
            
            os.makedirs(hf_local_path, exist_ok=True)
            print(f'Saving model param to {hf_local_path}')
            ...
            # 获取完整的模型状态字典并保存
            # NOTE: full_state_dict似乎是float32, AutoModel初始化时确实传入了Float32, 目前不知道其用意, 这里转换为bf16
            # print('Original full state dict type:', {k: v.dtype for k, v in full_state_dict.items() if isinstance(v, torch.Tensor)})
            full_state_dict = {k: v.to(torch.bfloat16) for k, v in full_state_dict.items() if isinstance(v, torch.Tensor)}
            self.model.save_pretrained(hf_local_path, state_dict=full_state_dict)
        torch.distributed.barrier()
        
        if not remove_previous_ckpt:
