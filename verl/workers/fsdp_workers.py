...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if role == 'actor' and optim_config is not None:
            if 'beta1' in optim_config and 'beta2' in optim_config:
                betas = (float(optim_config.beta1), float(optim_config.beta2))
                logger.info(f'Overwrite Actor optimizer betas: {betas}')
            else:
                betas = (0.9, 0.999)
            ...
            num_warmup_steps = optim_config.get('lr_warmup_steps', 0)
    ...
    def create_empty_tensor(self):
        ...
        self.empty_tensor = torch.empty(total_memory * 1024 * 1024 * 1024, dtype=torch.uint8, device='cuda')
    
    @register(dispatch_mode=Dispatch.ALL_TO_ALL)
    def del_empty_tensor(self):
        if self.empty_tensor is not None:
            del self.empty_tensor
            self.empty_tensor = None
            torch.cuda.empty_cache()
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        num_warmup_steps = config.optim.get('lr_warmup_steps', 0)
